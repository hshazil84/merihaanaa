...
async function getHomeData() {
  ...
  const [
    ...
    supabase
      .from("articles")
      .select("id, title, slug, excerpt, featured_image, cover_type, cover_video_thumbnail, category:categories!category_id(name, slug), author:authors!author_id(full_name)")
      .eq("status", "published")
      .eq("homepage_placement", "hero")
    ...
    supabase
      .from("articles")
      .select("id, title, slug, excerpt, featured_image, category:categories!category_id(name, slug)")
      .eq("status", "published")
      .eq("homepage_placement", "editors_choice")
      .order("published_at", { ascending: false })
      .limit(4),

    supabase
      .from("articles")
      .select("id, title, slug, excerpt, featured_image, category:categories!category_id(name, slug), author:authors!author_id(full_name)")
      .eq("status", "published")
      .eq("homepage_placement", "people")
      .order("published_at", { ascending: false })
      .limit(1),

    supabase
      .from("articles")
      .select("id, title, slug, excerpt, featured_image, reading_time_minutes, category:categories!category_id(name, slug)")
      .eq("status", "published")
      .eq("homepage_placement", "review")
      .order("published_at", { ascending: false })
      .limit(3),
              ...
              <Link key={article.id}
                ...
                {article.reading_time_minutes && (
                  <p className="mt-1.5" style={{
                    fontFamily: '"MVTypewriter", sans-serif',
                    fontSize: "10px", color: "rgb(160, 158, 152)", lineHeight: 2,
                  }}>
