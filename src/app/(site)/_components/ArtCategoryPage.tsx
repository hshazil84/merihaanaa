...
const CSS = [
  ".art-top{display:grid;grid-template-columns:1fr 300px;gap:1.5rem;align-items:stretch;}",
  ".art-hero{display:grid;grid-template-columns:1.1fr 1fr;gap:1.75rem;align-items:stretch;margin-bottom:1.5rem;}",
  ".art-3col{display:grid;grid-template-columns:1fr 1fr 1fr;gap:1.25rem;}",
  ".lc2{display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;}",
  ".lc4{display:-webkit-box;-webkit-line-clamp:4;-webkit-box-orient:vertical;overflow:hidden;}",
  "@media(max-width:1024px){.art-top{grid-template-columns:1fr!important;}}",
  "@media(max-width:768px){.art-hero{grid-template-columns:1fr!important;gap:1.25rem!important;}.art-3col{grid-template-columns:1fr 1fr!important;}}",
  "@media(max-width:480px){.art-3col{grid-template-columns:1fr!important;}}",
].join("");
...
function ArticleCard({ article, categorySlug }: { article: Article; categorySlug: string }) {
  ...
  return (
    <Link href={"/" + slug + "/" + article.slug} style={{ textDecoration: "none", display: "block" }}>
      ...
      {tag && <span style={{ fontFamily: FONT, fontSize: "9px", fontWeight: 700, color: VIOLET, border: "1px solid " + VIOLET, padding: "2px 8px", borderRadius: "20px", display: "inline-block", marginBottom: "5px" }}>{tag}</span>}
...
export default function ArtCategoryPage({ featured, articles, categorySlug }: Props) {
  return (
    <div style={{ backgroundColor: BG, minHeight: "100vh" }} dir="rtl">
      ...
      <header style={{ maxWidth: "84rem", margin: "0 auto", padding: "2rem 1.5rem 1.5rem", textAlign: "center" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "1rem" }}>
          ...
          <h1 style={{ fontFamily: FONT_DISPLAY, fontSize: "clamp(2rem,5vw,3.5rem)", color: VIOLET, lineHeight: 1.5, fontWeight: 400, margin: 0 }}>އާރޓް</h1>
          <span style={{ color: "rgba(0,0,0,0.18)", fontSize: "11px" }}>{"✦"}</span>
        </div>
      </header>
      ...
      <div style={{ maxWidth: "84rem", margin: "0 auto", padding: "0 1.5rem 4rem" }}>
        ...
        <div className="art-top" style={{ marginBottom: "2rem" }}>
              ...
              <div className="art-hero">
                ...
                <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", height: "100%" }}>
                  {getFirstTag(featured.tags) && (
                    <span style={{ fontFamily: FONT, fontSize: "10px", fontWeight: 700, color: VIOLET, border: "1.5px solid " + VIOLET, padding: "4px 12px", borderRadius: "20px", display: "inline-block", alignSelf: "flex-start", marginBottom: "12px", letterSpacing: "0.05em", lineHeight: 1.6 }}>
                      {getFirstTag(featured.tags)}
                    </span>
                  )}
                  ...
                  <Link href={"/" + (featured.category?.slug ?? categorySlug) + "/" + featured.slug}
                    style={{ fontFamily: FONT, fontSize: "12px", fontWeight: 700, color: VIOLET, textDecoration: "none", display: "inline-flex", alignSelf: "flex-start", alignItems: "center", gap: "4px", marginBottom: "16px", borderBottom: "1px solid rgba(122,74,163,0.4)", paddingBottom: "1px" }}>
          ...
          <div className="art-ad-rail">
            <Suspense fallback={null}>
              <AdSlot id="art-hero-rail" breakpoint="desktop" />
            </Suspense>
          </div>
        ...
        <div style={{ marginBottom: "2rem" }}>
          <Suspense fallback={null}>
            <AdSlot id="art-hero-rail" breakpoint="mobile" />
          </Suspense>
        </div>

        <div style={{ display: "flex", justifyContent: "center", paddingTop: "1rem", borderTop: "0.5px solid " + DIVIDER }}>
          <Link href={"/" + categorySlug + "/archive"}
            style={{ fontFamily: FONT, fontSize: "12px", fontWeight: 700, color: VIOLET, textDecoration: "none", padding: "8px 20px", border: "0.5px solid " + VIOLET, borderRadius: "8px" }}>
            {"އިތުރު ލިޔުންތައް ←"}
          </Link>
        </div>
