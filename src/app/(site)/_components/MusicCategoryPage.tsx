...
const CSS = [
  ".music-top{display:grid;grid-template-columns:1fr 300px;gap:1.5rem;align-items:stretch;}",
  ".music-hero{display:grid;grid-template-columns:1.1fr 1fr;gap:1.75rem;align-items:stretch;margin-bottom:1.5rem;}",
  ...
  "@media(max-width:1024px){.music-top{grid-template-columns:1fr!important;}}",
  "@media(max-width:768px){.music-hero{grid-template-columns:1fr!important;gap:1.25rem!important;}.music-3col{grid-template-columns:1fr 1fr!important;}}",
  "@media(max-width:480px){.music-3col{grid-template-columns:1fr!important;}}",
].join("");
...
export default function MusicCategoryPage({ featured, articles, categorySlug }: Props) {
  return (
    <div style={{ backgroundColor: BG, minHeight: "100vh" }} dir="rtl">
      ...
      <div style={{ maxWidth: "84rem", margin: "0 auto", padding: "0 1.5rem 4rem" }}>
        ...
        <div className="music-top" style={{ marginBottom: "2rem" }}>
          ...
          <div>
              ...
              <div className="music-hero">
                ...
                <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", height: "100%" }}>
                  {getFirstTag(featured.tags) && (
                    <span style={{ fontFamily: FONT, fontSize: "10px", fontWeight: 700, color: GREEN, border: "1.5px solid " + GREEN, padding: "4px 12px", borderRadius: "20px", display: "inline-block", alignSelf: "flex-start", marginBottom: "12px", letterSpacing: "0.05em", lineHeight: 1.6 }}>
                      {getFirstTag(featured.tags)}
                    </span>
                  )}
                  ...
                  {featured.excerpt && (
                    <p style={{ fontFamily: FONT, fontSize: "14px", color: TEXT_MUTED, lineHeight: 2, margin: "0 0 16px" }} className="lc4">{featured.excerpt}</p>
                  )}
                  <Link href={"/" + (featured.category?.slug ?? categorySlug) + "/" + featured.slug}
                    style={{ fontFamily: FONT, fontSize: "12px", fontWeight: 700, color: GREEN, textDecoration: "none", display: "inline-flex", alignSelf: "flex-start", alignItems: "center", gap: "4px", marginBottom: "16px", borderBottom: "1px solid rgba(113,188,34,0.4)", paddingBottom: "1px" }}>
            ...
            {articles.length > 0 && (
              <div className="music-3col">
                {articles.map(function(a) { return <ArticleCard key={a.id} article={a} categorySlug={categorySlug} />; })}
              </div>
            )}
          </div>

          <div className="music-ad-rail">
            <Suspense fallback={null}>
              <AdSlot id="music-hero-rail" breakpoint="desktop" />
            </Suspense>
          </div>
        ...
        <div style={{ marginBottom: "2rem" }}>
          <Suspense fallback={null}>
            <AdSlot id="music-hero-rail" breakpoint="mobile" />
          </Suspense>
        </div>

        <div style={{ display: "flex", justifyContent: "center", paddingTop: "1rem", borderTop: "0.5px solid " + DIVIDER }}>
          <Link href={"/" + categorySlug + "/archive"}
            style={{ fontFamily: FONT, fontSize: "12px", fontWeight: 700, color: GREEN, textDecoration: "none", padding: "8px 20px", border: "0.5px solid " + GREEN, borderRadius: "8px" }}>
            {"އިތުރު ލިޔުންތައް ←"}
          </Link>
        </div>
