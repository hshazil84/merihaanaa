// src/app/(site)/originals/page.tsx
...
function OriginalCard({ item, large = false }: { item: any; large?: boolean }) {
  return (
    <Link href={`/originals/${item.slug}`} className="group block flex-none">
      <div className={`relative ${large ? "aspect-video" : "aspect-video"} rounded-xl overflow-hidden bg-neutral-900`}>
        {item.thumbnail_url ? (
          <img
      ...
      <p
        className="mt-2 text-sm font-semibold text-neutral-100 line-clamp-2 leading-snug group-hover:text-white transition-colors"
        style={{ fontFamily: "MVTypewriter, serif", direction: "rtl" }}
      >
        {item.title}
      </p>
      {item.series && (
        <p className="text-xs text-neutral-500 mt-0.5" style={{ fontFamily: "MVTypewriter, serif", direction: "rtl" }}>
          {item.series.title}
        </p>
      )}
    </Link>
...
export default async function OriginalsPage() {
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      {featured && (
        <div className="relative w-full aspect-[21/9] md:aspect-[3/1] overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 md:p-12" dir="rtl">
            <span
              className="inline-block text-xs px-2.5 py-1 rounded-full bg-white/10 text-white/70 mb-3 border border-white/10"
              style={{ fontFamily: "MVTypewriter, serif" }}
            >
              {TYPE_LABELS[featured.type] ?? featured.type}
            </span>
            <h1
              className="text-2xl md:text-4xl font-bold text-white mb-3 max-w-2xl leading-snug"
              style={{ fontFamily: "MVTypewriter, serif" }}
            >
              {featured.title}
            </h1>
            {featured.description && (
              <p
                className="text-sm md:text-base text-neutral-300 max-w-xl line-clamp-2 mb-5"
                style={{ fontFamily: "MVTypewriter, serif" }}
              >
                {featured.description}
              </p>
      ...
      {/* Content rows */}
      <div className="px-4 md:px-8 pb-16 space-y-10 mt-8">
        {!hasContent && (
          <div className="text-center py-24 text-neutral-600" style={{ fontFamily: "MVTypewriter, serif" }}>
            ވިޑިއޯ ނެތް
          </div>
        )}
          ...
          <section key={typeKey}>
            ...
            >
              {grouped[typeKey].map((item: any) => (
                <div
                  key={item.id}
                  className="flex-none w-[80vw] sm:w-[44vw] md:w-[30vw] lg:w-[22vw] snap-start"
                >
