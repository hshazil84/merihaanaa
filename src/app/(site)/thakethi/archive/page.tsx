...
export default async function ThakethiArchivePage({ searchParams }: PageProps) {
    ...
    <div style={{ backgroundColor: BG, minHeight: "100vh" }} dir="rtl">
      ...
      <header style={{ maxWidth: "84rem", margin: "0 auto", padding: "1.5rem 1.5rem 0" }}>
        <div style={{ fontFamily: FONT, fontSize: "12px", color: TEXT_MUTED, display: "flex", alignItems: "center", gap: "6px", justifyContent: "center" }}>
          <Link href="/thakethi" style={{ color: TEXT_MUTED, textDecoration: "none" }}>{category.name}</Link>
          <span>{"<"}</span>
          <span style={{ color: OCHRE, fontWeight: 700 }}>ލިޔުންތައް</span>
        </div>
      </header>
      ...
      <header style={{ maxWidth: "84rem", margin: "0 auto", padding: "1rem 1.5rem 1.5rem", textAlign: "center" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "1rem" }}>
          ...
          <h1 style={{ fontFamily: FONT_DISPLAY, fontSize: "clamp(2rem,5vw,3.5rem)", color: OCHRE, lineHeight: 1.5, fontWeight: 400, margin: 0 }}>
          ...
          <>
            ...
            {totalPages > 1 && (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", paddingTop: "2rem", marginTop: "2rem", borderTop: "0.5px solid " + DIVIDER }}>
                {page > 1 && (
                  <a href={"/thakethi/archive?page=" + (page - 1)} style={{ fontFamily: FONT, fontSize: "12px", fontWeight: 700, color: OCHRE, textDecoration: "none", padding: "6px 14px", border: "0.5px solid " + OCHRE, borderRadius: "6px" }}>
                    {"← ކުރީ"}
                  </a>
                )}
