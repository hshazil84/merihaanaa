...
export default async function MeehunArchivePage({ searchParams }: PageProps) {
    ...
    <div style={{ backgroundColor: BG, minHeight: "100vh" }} dir="rtl">
      ...
      <header style={{ maxWidth: "84rem", margin: "0 auto", padding: "1.5rem 1.5rem 0" }}>
        <div style={{ fontFamily: FONT, fontSize: "12px", color: TEXT_MUTED, display: "flex", alignItems: "center", gap: "6px", justifyContent: "center" }}>
          <Link href="/meehun" style={{ color: TEXT_MUTED, textDecoration: "none" }}>{category.name}</Link>
          <span>{"<"}</span>
          <span style={{ color: CORAL, fontWeight: 700 }}>ލިޔުންތައް</span>
        </div>
      </header>
          ...
          <>
            ...
            {totalPages > 1 && (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", paddingTop: "2rem", marginTop: "2rem", borderTop: "0.5px solid " + DIVIDER }}>
                {page > 1 && (
                  <a href={"/meehun/archive?page=" + (page - 1)} style={{ fontFamily: FONT, fontSize: "12px", fontWeight: 700, color: CORAL, textDecoration: "none", padding: "6px 14px", border: "0.5px solid " + CORAL, borderRadius: "6px" }}>
                    {"← ކުރީ"}
                  </a>
                )}
