...
const STAGES: { id: string; label: string; color: string; bg: string; statuses: string[] }[] = [
  { id: "idea",      label: "Idea",      color: "#94a3b8", bg: "#f8fafc", statuses: [] },
  { id: "draft",     label: "Draft",     color: "#f59e0b", bg: "#fffbeb", statuses: ["draft"] },
  { id: "scheduled", label: "Scheduled", color: "#3b82f6", bg: "#eff6ff", statuses: ["scheduled"] },
  { id: "published", label: "Published", color: "#22c55e", bg: "#f0fdf4", statuses: ["published"] },
];
        ...
        {days.map((day) => {
          ...
          return (
            <div key={key} className={"rounded-lg p-1.5 min-h-[72px] border transition-colors " + (isToday ? "border-blue-200 bg-blue-50" : "border-gray-100 bg-white hover:border-gray-200")}>
              <p className={"text-[11px] font-semibold mb-1 " + (isToday ? "text-blue-600" : "text-gray-500")}>
                {day.getDate()}
              </p>
              <div className="space-y-0.5">
                ...
                {items.length > 2 && (
                  <p className="text-[9px] text-gray-400 px-1">+{items.length - 2} more</p>
                )}
              </div>
            </div>
          );
...
function GanttView({ articles }: { articles: Article[] }) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex border-b border-gray-100 mb-0 overflow-hidden" style={{ marginLeft: "200px" }}>
        <div className="flex flex-shrink-0" style={{ width: days * colW + "px" }}>
          {cols.map((d, i) => (
      ...
      <div className="flex-1 overflow-y-auto">
        {withDates.map((article) => {
          const start = dayOffset(article.created_at);
          const end = dayOffset(article.scheduled_at ?? article.published_at);
          const left = Math.max(0, start) * colW;
          const width = Math.max(colW, (end - Math.max(0, start)) * colW);
          ...
          return (
            <div key={article.id} className="flex items-center border-b border-gray-50 hover:bg-gray-50 transition-colors" style={{ height: "40px" }}>
              ...
              <div className="relative flex-1 overflow-hidden" style={{ height: "40px" }}>
                ...
                {/* Bar */}
                <div className="absolute top-1/2 -translate-y-1/2 rounded-md flex items-center px-2 cursor-pointer hover:opacity-80 transition-opacity"
...
// ── Main ─────────────────────────────────────────────────────────────────────
export default function ProductionClient({ articles: initial }: { articles: Article[] }) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-gray-50/50">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <input value={filter} onChange={(e) => setFilter(e.target.value)}
              ...
              className="text-xs border border-gray-200 rounded-lg px-3 py-1.5 w-48 outline-none focus:border-gray-400 transition-colors bg-gray-50" />
      ...
      <div className="flex-1 overflow-hidden p-5">
        {view === "kanban" && (
          <KanbanView articles={filtered} onStageChange={handleStageChange} />
        )}
        {view === "calendar" && (
