    ...
    <section className="px-6 py-12" style={{ backgroundColor: "#f7f1e2" }} dir="rtl">
      <div className="max-w-6xl mx-auto">
        ...
        <h2 className="text-center mb-10" style={{
          ...
          fontWeight: 400, fontSize: "26px", color: "rgb(26, 26, 26)", lineHeight: 2,
        }}>
          ރިވިއު
        </h2>
        ...
        <div className="hidden md:grid md:grid-cols-3">
          {articles.slice(0, 3).map((article, i) => {
            const image = article.cover_portrait_url ?? article.featured_image;
            return (
              <>
                {i > 0 && (
                  <div key={"div-" + i} className="absolute" style={{ display: "none" }} />
                )}
                <div key={article.id} className="relative">
                  {i > 0 && (
                    <div className="absolute top-0 right-0 bottom-0 w-px" style={{ background: "rgb(210, 207, 200)" }} />
                  )}
                  <Link
                  ...
                  >
                    ...
                    <h3 className="line-clamp-2 group-hover:opacity-70 transition-opacity" style={{
                      fontFamily: '"MVTypewriter", "Noto Sans Thaana", sans-serif',
                      fontWeight: 700, fontSize: "16px", color: "rgb(26, 26, 26)", lineHeight: 2,
                    }}>
                      {article.title}
                    </h3>
                    {article.excerpt && (
                      <p className="mt-1.5 line-clamp-3" style={{
                        fontFamily: '"MVTypewriter", "Noto Sans Thaana", sans-serif',
                        fontWeight: 400, fontSize: "13px", color: "rgb(110, 108, 102)", lineHeight: 2,
                      }}>
                        {article.excerpt}
                      </p>
                    )}
                    {article.reading_time_minutes && (
                      <p className="mt-2" style={{
                        fontFamily: '"MVTypewriter", "Noto Sans Thaana", sans-serif',
                        fontWeight: 400, fontSize: "10px", color: "rgb(160, 158, 152)", lineHeight: 2,
                      }}>
        ...
        <div className="md:hidden flex gap-4 overflow-x-auto pb-2 no-scrollbar" style={{ scrollbarWidth: "none" }}>
          {articles.slice(0, 3).map((article) => {
            const image = article.cover_portrait_url ?? article.featured_image;
            return (
              <Link key={article.id}
                href={"/" + (article.category?.slug ?? "article") + "/" + article.slug}
                className="group flex flex-col items-center text-center flex-shrink-0 w-[58vw]"
              >
                <div className="w-full aspect-[3/4] overflow-hidden mb-3 rounded-lg bg-[#e8e5de]">
                  {image ? (
                    <img src={image} alt={article.title}
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  ) : (
