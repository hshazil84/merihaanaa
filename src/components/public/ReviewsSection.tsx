  ...
  return (
    <section className="px-6 py-12" style={{ backgroundColor: "#f7f1e2" }} dir="rtl">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Desktop: 3 columns with dividers | Mobile: horizontal scroll */}
        ...
        <div className="md:hidden flex gap-5 overflow-x-auto pb-2 no-scrollbar" style={{ scrollbarWidth: "none" }}>
          {articles.slice(0, 3).map((article) => (
            <Link key={article.id}
              href={`/${article.category?.slug ?? "article"}/${article.slug}`}
              className="group flex flex-col items-center text-center flex-shrink-0 w-[75vw]"
            >
              <div className="w-full aspect-[3/4] overflow-hidden mb-4 rounded-lg bg-[#e8e5de]">
                {article.featured_image ? (
                  <img src={article.featured_image} alt={article.title}
